...
# Set server port for Railway (optional for local run, required on Railway)
...
def main():
    ...
    model = load_model()
    if model is None:
        st.error("Model not found. Please train or upload the model file.")
        return
    ...
    if uploaded_file is not None:
        try:
            ...
            expected_features = 30  # Time, Amount, V1–V28
            if len(df.columns) != expected_features and 'Class' not in df.columns:
                st.error(f"Input data must have exactly {expected_features} features (Time, Amount, V1–V28)")
                return
